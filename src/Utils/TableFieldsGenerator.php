...
namespace JJSoft\Generator\Utils;

use DB;

class TableFieldsGenerator
{
    public static function generateFieldsFromTable($tableName,$connectionName)
    {
        ...
        $primaryKey = static::getPrimaryKeyFromTable($tableName,$connectionName);
        $timestamps = static::getTimestampFieldNames();
        $defaultSearchable = config('jjsoft.jjsoft_generator.options.tables_searchable_default', false);
     ...
     */
    public static function getPrimaryKeyFromTable($tableName,$connectionName)
    {
        $schema = DB::connection($connectionName)->getDoctrineSchemaManager();
    ...
    {
        if (!config('jjsoft.jjsoft_generator.timestamps.enabled', true)) {
            return [];
        }

        $createdAtName = config('jjsoft.jjsoft_generator.timestamps.created_at', 'created_at');
        $updatedAtName = config('jjsoft.jjsoft_generator.timestamps.updated_at', 'updated_at');
