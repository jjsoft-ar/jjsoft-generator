...
class APIRoutesGenerator extends BaseGenerator
{
    ...
    public function __construct(CommandData $commandData)
    {
        ...
        if (!empty($this->commandData->config->prefixes['route'])) {
            $routesTemplate = TemplateUtil::getTemplate('api.routes.prefix_routes', 'jjsoft-generator');
        } else {
            $routesTemplate = TemplateUtil::getTemplate('api.routes.routes', 'jjsoft-generator');
        }
