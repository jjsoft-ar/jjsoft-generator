...
class LayoutPublishCommand extends PublishBaseCommand
{
     ...
     * @var string
     */
    protected $name = 'jjsoft.publish:layout';
    ...
    {
        $viewsPath = config('jjsoft.jjsoft_generator.path.views', base_path('resources/views/'));
        ...
        foreach ($files as $stub => $blade) {
            $sourceFile = base_path('vendor/jjsoft/'.$templateType.'/templates/scaffold/'.$stub);
    ...
    private function updateRoutes()
    {
        ...
        $routesTemplate = TemplateUtil::getTemplate('routes.auth', 'jjsoft-generator');
        if ($this->laravelVersion == '5.1') {
    ...
    {
        $templateData = TemplateUtil::getTemplate('home_controller', 'jjsoft-generator');
    ...
    private function fillTemplate($templateData)
    {
        ...
        $templateData = str_replace(
            '$NAMESPACE_REQUEST$',
            config('jjsoft.jjsoft_generator.namespace.request'), $templateData
        );
